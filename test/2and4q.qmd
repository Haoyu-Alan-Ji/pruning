...
Q_cur[Q_cur!=0] <- exp(log_trans_rates)  ## fill in transition rate values
...
```{r test-prune-nll}
## test
pars <- list(log_trans_rates=log(c(1.2, 0.5)))
n <- length(pars$log_trans_rates)
factors <- which(n %% 1:n == 0)
dim <- factors[length(factors)/2 + 1]
Q <- matrix(0, dim, dim)
non_diag <- (row(Q) != col(Q))
Q[non_diag] <- pars$log_trans_rates ## fill in off-diagonals
diag(Q) <- -1*rowSums(Q)
...
pars <- list(log_trans_rates=log(c(0.25, 0.75)))
prune_nll(pars)
...
```{r}
Q_fit <- setup_Q(exp(fit_4by4$par), Q_template4)
Matrix(expm(Q_fit))
```
...
data(primates)
head(primates)
corHMM_fit0 <- corHMM(p1, primate.discrete2, rate.cat = 1)
## try fitting this data set in prune_nll??
## what are the differences in procedure between corHMM and our stuff?
##   priors on root?
##   other regularization etc.?

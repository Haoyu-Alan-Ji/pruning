...
RTMB [@RTMBPkg] shows a slight improvement in the calculation of negative log-likelihood (this is more noticeable when the tree length is small). 
...
#| code-summary: "reveal in propotion"
...
Now we plot comp-time and neg-loglik together. For the same seed and ntaxa, we compare the results from orig-corHMM and RTMB, and select the result with the smaller nll (nll winner). This result is then put on a scatter plot with ntaxa on the x-axis and comp-time on the y-axis. The size of the point represents the advantage of the nll winner over the other method in that case. Notice that RTMB shows better linearity when there were more nll winners, and this situation goes better when ntaxa increases. Such comparision implys RTMB has more stable computational performance.

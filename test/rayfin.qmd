...
library(here)
## setwd(here::here()) ## BMB: should be unnecessary as long as you use here() throughout
```
...
- In `Q_template.R`, `Q_template()` can be used to generate the transition matrix $Q$, `simfun()` can output trees of arbitrary size and specify the desired trait matrix for generation (currently only supports binary states).
- In `getinfo.R`, `sumfun()` can run corHMM quietly, allowing users to choose between using RTMB or the original version, and extract information (computation time, log-likelihood, and predicted values, etc.).
- `RealTree.R` provides a method for conducting large-scale benchmarking by sampling from real-world datasets.

The dataset is composed of traits + a phylogeny on ray-finned fishes [@eckert_parental_2025]
```{r realdat}
...
RTMB [@RTMBPkg] shows a slight improvement in the calculation of negative log-likelihood (this is more noticeable when the tree length is small). The mean difference across all simulations is that RTMB gets a result that is `r round(tt$estimate, 2)` log-likelihood units better on average (95% CI `r round(tt$conf.int[1], 2)` - `r round(tt$conf.int[2], 2)`), with an interquartile (0.25 to 0.75) range from `r round(qq[1], 2)` LL units better to `r round(qq[2], 2)` LL units worse. (We haven't tried this with multiple random starting values yet.) These results are for the ARD (all rates different) model with 3 binary traits (24 parameters; no non-trivial covarion-model examples yet).
...
ggplot(dd, aes(x = ntaxa_bin, y = ldiff)) +
  ...
  labs(
    ...
    y = "difference in log-likelihood\n(RTMB - original)"
  ) +
  theme_minimal()
```
...
Then check all computing time results.
...
Group the data according to ntaxa, and use median to represent the ratio of RTMB and orig_corHMM computation times under normal circumstances. There was approximately a 50 to 100-fold improvement, and this ratio increased with increasing ntaxa.
...
Now we plot computation time and negative log likelihood results together. For the same seed and ntaxa, we compare the results from original corHMM and RTMB, and select the result with the smaller NLL ("nll winner"). This result is then put on a scatter plot with ntaxa on the x-axis and comp-time on the y-axis. The size of the point represents the advantage of the nll winner over the other method in that case. Notice that RTMB shows better linearity when there were more nll winners, and this situation goes better when ntaxa increases. This comparison implies RTMB has more stable computational performance.

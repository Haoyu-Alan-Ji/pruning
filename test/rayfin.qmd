...
RTMB [@RTMBPkg] shows a slight improvement in the calculation of negative log-likelihood (this is more noticeable when the tree length is small). When calculating larger trees, RTMB also demonstrates more stable computational performance, which can be observed through the smaller variance in computation time.
